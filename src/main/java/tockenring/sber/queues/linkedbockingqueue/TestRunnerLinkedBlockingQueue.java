...
public class TestRunnerLinkedBlockingQueue {
...
/*        for (int i = 256; i <= 1056; i = i + 100) {///nodes
    ...
    private static void runTest(int nodeCount, int contentCount) {
        ...
        try (FileWriter testNameFile = new FileWriter("testdata/testName.txt", true);
            ...
            testNameFile.append("LinkedBlockingQueue" + '\n');
            testNameFile.flush();
        ...
        TokenRingLinkedBlockingQueue tokenRing = new TokenRingLinkedBlockingQueue(nodeCount);
        ...
        tokenRing = new TokenRingLinkedBlockingQueue(nodeCount);
        tokenRing.fillContent(contentCount);
        tokenRing.startThreads();
    ...
    private static void generateThroughput(int nodeCount, int contentCount, TokenRingLinkedBlockingQueue tokenRing, long time) {
    ...
    private static void generateLatency(int nodeCount, int contentCount, TokenRingLinkedBlockingQueue tokenRing) {
        try (FileWriter minLatency = new FileWriter("testdata/MinLatency.txt", true);
            ...
            Comparator comparator = Comparator.comparingLong(Long::longValue);
            TokenNodeLinkedBlockingQueueTimeStampChecker timeStampChecker = (TokenNodeLinkedBlockingQueueTimeStampChecker)
